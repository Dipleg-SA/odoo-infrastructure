...
`<chatter/>` is self-closing, placed as a sibling **after** `</sheet>` — it never wraps the sheet:
...
> **Verified against real views in `.repos/odoo`** (`addons/account/views/account_account_views.xml` and others): every real usage is the self-closing sibling form above. `<chatter>` wrapping `<sheet>` doesn't appear anywhere in the codebase.
...
> **Verified against `odoo/addons/base/models/ir_ui_view.py:493-497`**: `attrs` and `states` raise `ValidationError` at view-load time — *"Since 17.0, the 'attrs' and 'states' attributes are no longer used."* Use direct-expression `invisible="..."` instead, as shown above.
...
- Verified against `.repos/odoo` (community, `version_info = (19, 0, 0, FINAL, 0, '')`): `odoo/addons/base/models/ir_ui_view.py`, `addons/web/static/src/views/utils.js`, `addons/web/static/src/views/graph/`, `addons/account/views/`.
