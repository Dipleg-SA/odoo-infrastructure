...
> **Verified against `odoo/modules/module.py` (`adapt_version`)**: if the version has 3 parts or fewer and doesn't already start with the current series, Odoo **auto-prefixes it with the series**. `'1.0.0'` on Odoo 19 is stored/displayed as `'19.0.1.0.0'`. `check_version` requires the final (adapted) version to start with the series — don't be surprised to see the series prefix appear in the Apps list even though you only wrote `'1.0.0'` in the manifest.
...
- Verified against `.repos/odoo` (community, `version_info = (19, 0, 0, FINAL, 0, '')`): `odoo/modules/module.py`, `odoo/modules/loading.py`, `odoo/addons/base/models/ir_module.py`.
