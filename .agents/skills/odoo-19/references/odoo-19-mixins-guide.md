...
- `message_type` (str): e.g. `notification`, `comment`, `email` — anything except `user_notification` (reserved for `message_notify`)
- `subtype_xmlid` (str): XML id of a `mail.message.subtype` to fetch, forces `subtype_id` — **there is no plain `subtype` parameter**, only `subtype_xmlid`/`subtype_id`
- `subtype_id` (int): subtype id, used mainly for follower notification
- `parent_id` (int): Reply to message
- `attachments` (list): List of `(name, content)` or `(name, content, info)` tuples, content NOT base64-encoded
- `outgoing_email_to` / `incoming_email_to` / `incoming_email_cc` (str): comma-separated email lists — **new in Odoo 19** (marked experimental in the source docstring)
- `**kwargs`: Extra mail.message field values

> **Verified against `addons/mail/models/mail_thread.py`**: the real signature has no `subtype` parameter — only `subtype_xmlid`/`subtype_id`, exactly as used in the code example above.
...
> **Verified against `addons/mail/models/mail_thread.py`**: `tracking` doesn't support a string value to scope tracking to a specific subfield — no such mechanism exists in the source. It's read as a plain truthy check (`getattr(field, 'tracking', None)`); the only documented non-boolean use is an **int** to order multiple tracked fields relative to each other in the chatter (e.g. `tracking=10`), not to select a subfield. For a `Many2one`, tracking logs the field's own value change (old related record → new related record), not an arbitrary subfield of the related record.
...
# Get rating stats
stats = record.rating_get_stats()
avg_rating = stats['avg']
```

> **Verified against `addons/rating/models/rating_mixin.py:175-192`**: `rating_get_stats()` returns a **dict** (`{'avg': ..., 'total': ..., 'percent': {1: ..., ..., 5: ...}}`), not the average by itself — access `['avg']` for that.
...
- Verified against `.repos/odoo` (community, `version_info = (19, 0, 0, FINAL, 0, '')`): `addons/mail/models/mail_thread.py`, `addons/mail/static/src/chatter/web/form_compiler.js`, `addons/rating/models/rating_mixin.py`.
