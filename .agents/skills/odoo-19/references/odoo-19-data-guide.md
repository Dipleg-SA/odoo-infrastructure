...
- `time`, `datetime`, `DateTime`, `timedelta`, `relativedelta`, `pytz` modules
- `ref()` function to resolve external IDs
- `Command` (for relational field operations, e.g. `eval="[Command.link(ref('base.res_partner_1'))]"`)
- `version` (current Odoo major version)
...
\*At least one of `id` or `search` is required — but they are **not exclusive**: if both are given, the records to delete are the **union** of the search results and the record matching `id` (`records = records.search(...)` then `records += records.browse(...)` in `convert.py`).
...
- File name: `{model_name}.csv` or `{model_name}-suffix.csv` — the model is derived by splitting the filename on the **first** `-` (e.g. `res.country.state-custom.csv` still targets `res.country.state`). Useful when two modules need to ship CSV data for the same model without filename collisions.
- First row: fields to write, special field `id` for external IDs
- Each row: creates **or updates** a record — CSV import goes through the same `load()` method as the UI import wizard, so a row whose `id` already exists as an external ID **updates** that record instead of creating a duplicate (same create-or-update semantics as the `record` tag, not create-only).
...
- Verified against `.repos/odoo` (community, `version_info = (19, 0, 0, FINAL, 0, '')`): `odoo/tools/convert.py`.
