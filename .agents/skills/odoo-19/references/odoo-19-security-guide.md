...
> **Verified against `odoo/addons/base/models/ir_rule.py:38-50`**: `_eval_context()` only returns these 3 variables — `time` is **not** available in `domain_force`. `domain_force` is an expression (not a statement), so there's no way to `import time` inside it either.
...
- Verified against `.repos/odoo` (community, `version_info = (19, 0, 0, FINAL, 0, '')`): `odoo/addons/base/models/res_groups.py`, `odoo/addons/base/models/res_groups_privilege.py`, `odoo/addons/base/models/ir_rule.py`, `odoo/tools/misc.py`.
