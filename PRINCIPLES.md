...
- **Obligatorio.** En `runtime/<entorno>/compose.env` va **solo lo que Compose necesita interpolar**. Todo lo demás va literal en el archivo de config de la herramienta que lo usa. La pregunta "¿dónde va este valor?" tiene entonces una respuesta mecánica y no un juicio.
