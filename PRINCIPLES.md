...
- **Obligatorio.** Parametrizá por `.env` todo valor que varía por deployment y se use dentro de un `docker/compose.*.yaml`. Cuando el valor vive en el archivo de config propio de una herramienta, tiene que llegar por el mecanismo que **esa herramienta** ofrezca: env vars que pisan el archivo, interpolación en su propio provisioning, o un entrypoint que lo appendea al conf de runtime. Compose no interpola dentro de archivos bind-mounted.
...
- Módulos de Compose: `docker/compose.<capa>.yaml`, uno por capa, y un **entrypoint por entorno** que elige cuáles incluir. `docker/compose.yaml` es el de producción; `docker/compose.staging.yaml` y `docker/compose.dev.yaml` son entrypoints hermanos, no módulos, y se seleccionan con `COMPOSE_FILE` en `.env`.
...
- Dockerfiles propios y sus contextos: un directorio por servicio bajo `docker/`, al lado de los archivos de Compose que los construyen.
- Árbol de addons: `addons/.repos/<repo>.git` para el clon bare y `addons/<categoría>/<repo>` para cada worktree. **Un árbol por checkout**, no un subdirectorio por entorno: qué entorno es lo dice `.env`, y superponer los dos aislamientos deja uno que no aísla nada. Todo `addons/` va gitignoreado; lo único versionado son las dos plantillas, la del manifiesto y la de los pines Python que se derivan de él (`addons/requirements.txt`). Las dos se bootstrapean con `cp` y ninguno de los dos archivos reales se versiona: son función de qué addons trae cada deployment.
...
- Los archivos de Compose viven bajo `docker/` y `.env` en la **raíz**: `COMPOSE_FILE` los nombra desde ahí, así que ninguna invocación lleva flags. `.env` no se mueve — Compose lo lee del directorio donde se corre el comando, no del que contiene el archivo elegido. Al no quedar ningún `compose.yaml` auto-descubrible, un checkout sin `.env` falla con `no configuration file provided` en vez de resolver producción con variables vacías.
