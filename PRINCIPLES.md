...
- **Obligatorio.** Un stack es **un contenedor con cosas propias**, todas en su carpeta: imagen, config, scripts, units y verificación. Que corra siempre o a demanda no decide nada — certbot vive treinta segundos por corrida y tiene imagen, secret, unit y chequeos suyos. Lo que **no** es un stack es lo que no tiene nada propio: eso vive dentro del que sirve, porque darle carpeta sería inventarle andamiaje.
...
- **Obligatorio.** En `runtime/<entorno>/compose.env` va **solo lo que Compose necesita interpolar y los selectores compartidos del runtime**, como `ADDONS_REF`. Todo lo demás va literal en el archivo de config de la herramienta que lo usa. La pregunta "¿dónde va este valor?" tiene entonces una respuesta mecánica y no un juicio.
...
- **Obligatorio.** La inicialización automática de ramas solo puede ocurrir desde desarrollo y sobre referencias `feat/*`. Staging y producción consumen ramas protegidas con credenciales de solo lectura; la diferencia se garantiza en el proveedor Git, no con un condicional local.
...
- **Obligatorio.** Operar módulos en la base es un paso **explícito del operador**, nunca atado al arranque del contenedor. Una actualización de varios minutos disparada en cada boot se repite en el restart automático de un crash, alargando la caída en vez de resolverla.
- **Obligatorio.** Publicar un candidato de addons solo prepara código. Aplicarlo exige que el operador recree Odoo; instalar, actualizar o desinstalar módulos sigue siendo una acción manual separada.
- **Obligatorio.** Registrá en cada arranque qué commits y árboles cargó Odoo. Verificación, promoción y backup se apoyan en esa selección ejecutada, nunca en el candidato que casualmente esté en disco al consultarlo.
...
- **Obligatorio.** Separá la imagen del código montado: la imagen contiene Odoo y dependencias compiladas; los addons llegan por binds de solo lectura propios de cada entorno. Un cambio de código no reconstruye la imagen, pero una nueva base, edición o huella de dependencias sí.
...
- Identidad del stack: `COMPOSE_PROJECT_NAME` y las variables de cada entorno viven en `runtime/<entorno>/compose.env`, igual en todos los entornos. Cada archivo de Compose declara su `name:` derivado de esa identidad. Del nombre derivan `container_name`, volúmenes, redes y **tags de imagen**.
- Valores por deployment: `runtime/<entorno>/compose.env` no versionado, con una plantilla versionada junto a la composición y sin completar. Son copias asumidas: se elige que copiar sea `cp` y nada más, en vez de borrar bloques de un archivo genérico.
...
- Árbol de addons: un clon bare por repo bajo `runtime/addons/.repos/` y candidatos ejecutables separados bajo `runtime/<entorno>/addons/{custom,enterprise}`. Cada Odoo monta solo los suyos como solo lectura; el catálogo y los overrides Python compartidos permanecen bajo `runtime/addons/`.
