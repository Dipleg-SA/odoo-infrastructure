...
- Árbol de addons: `addons/.repos/<repo>.git` para el clon bare y `addons/<entorno>/<categoría>/<repo>` para cada worktree, con `<entorno>` en `{production, staging, development}`. El servidor materializa los dos primeros; el de desarrollo es opt-in y solo existe en la máquina del operador. Todo `addons/` va gitignoreado; el único artefacto versionado es el manifiesto.
