...
**Dynamic selection** (values computed at runtime, not from another model — a `Many2one` is a different, unrelated field type):

```python
status = fields.Selection(selection='_selection_status', string="Status")
...
# Add/subtract
next_week = date_utils.add(Date.today(), weeks=1)
last_month = date_utils.subtract(Date.today(), months=1)

# Start/end of period
start_of_month = date_utils.start_of(Date.today(), 'month')
end_of_month = date_utils.end_of(Date.today(), 'month')
...
> **Verified against `odoo/orm/fields_temporal.py`**: `add`, `subtract`, `start_of`, `end_of` are **not** methods on `fields.Date` — the `Date` class only defines `today`, `context_today`, `to_date`, `to_string`. Those four functions live in `odoo.tools.date_utils`. Calling `Date.add(...)` raises `AttributeError`.
...
# Add/subtract
next_hour = date_utils.add(Datetime.now(), hours=1)

# Start/end of period
start_of_day = date_utils.start_of(Datetime.now(), 'day')
end_of_day = date_utils.end_of(Datetime.now(), 'day')
...
> Same nuance as `Date` above: `add`/`start_of`/`end_of` come from `odoo.tools.date_utils`, not from `fields.Datetime` itself.
...
| `tracking` | Track changes in chatter — `True`/`False`, or an int for ordering multiple tracked fields |
...
> **Verified**: `track_visibility` does not exist anywhere in Odoo 19 (`grep -rn "track_visibility" addons/mail/ odoo/orm/` → 0 matches) — it was replaced by `tracking=` several versions ago. `tracking` is the only valid parameter now.
```
...
- Verified against `.repos/odoo` (community, `version_info = (19, 0, 0, FINAL, 0, '')`): `odoo/orm/fields_temporal.py`, `odoo/orm/fields_numeric.py`, `odoo/orm/fields_selection.py`, `odoo/orm/commands.py`, `odoo/orm/fields.py`, `odoo/tools/date_utils.py`, `addons/mail/models/mail_thread.py`.
