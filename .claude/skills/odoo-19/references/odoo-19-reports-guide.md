...
| `web.external_layout_standard`  | Standard variant                     |
| `web.external_layout_boxed`     | Boxed layout                         |
| `web.external_layout_bold`      | Bold layout                          |
| `web.external_layout_striped`   | Striped layout                       |
| `web.external_layout_folder`    | Folder layout                        |
| `web.external_layout_wave`      | Wave layout                          |
| `web.external_layout_bubble`    | Bubble layout                        |

> **Verified against `addons/web/views/report_templates.xml`**: these 8 are the real layout templates. `web.external_layout_background` and `web.external_layout_clean` don't exist.
...
> **Verified against `odoo/addons/base/data/report_paperformat_data.xml`**: only these two built-in paper formats exist. `paperformat_us_legal` doesn't exist anywhere in the codebase — create a custom `report.paperformat` record (see below) if you need Legal size.
...
```xml
<img t-att-src="'/report/barcode/?barcode_type=%s&amp;value=%s&amp;width=%s&amp;height=%s' % ('QR', o.name, 200, 200)"/>
```

> **Verified against `addons/web/controllers/report.py:55-58`**: the query parameter is `barcode_type`, **not** `type` — the controller signature is `report_barcode(self, barcode_type, value, **kwargs)`. Using `type=` leaves `barcode_type` unsatisfied and the request fails.

### Barcode Controller Parameters

| Parameter       | Values                                                                                                                                    |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `barcode_type`  | `Codabar`, `Code11`, `Code128`, `EAN13`, `EAN8`, `Extended39`, `Extended93`, `FIM`, `I2of5`, `MSI`, `POSTNET`, `QR`, `Standard39`, `Standard93`, `UPCA`, `USPS_4State` |
| `value`         | Barcode value                                                                                                                             |
| `width`         | Width in pixels                                                                                                                           |
| `height`        | Height in pixels                                                                                                                           |
| `humanreadable` | `1` or `0` (show text below)                                                                                                              |

> **Verified against `addons/web/controllers/report.py` docstring**: `UPCE` is **not** a valid `barcode_type` — the real list is the 16 values above.
...
<div class="page">
    ...
    <img
        t-att-src="'/report/barcode/?barcode_type=%s&amp;value=%s&amp;width=%s&amp;height=%s&amp;humanreadable=1' % ('EAN13', o.barcode, 600, 100)"
        alt="Barcode"/>
</div>
```
...
- Verified against `.repos/odoo` (community, `version_info = (19, 0, 0, FINAL, 0, '')`): `odoo/addons/base/models/ir_actions_report.py`, `odoo/addons/base/models/report_paperformat.py`, `odoo/addons/base/data/report_paperformat_data.xml`, `addons/web/views/report_templates.xml`, `addons/web/controllers/report.py`, `odoo/addons/base/models/ir_qweb.py`.
