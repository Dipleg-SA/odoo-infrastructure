...
There is no separate `_name` argument to omit — `Constraint(definition, message='')` doesn't accept a name parameter at all. The constraint's name **is** the Python attribute name it's assigned to (must start with `_`), always, not an auto-generated fallback.

> **Verified against `odoo/orm/table_objects.py:85`**: `Constraint.__init__(self, definition, message='')` — no `name` parameter exists to omit.
...
- Verified against `.repos/odoo` (community, `version_info = (19, 0, 0, FINAL, 0, '')`): `odoo/orm/models.py`, `odoo/orm/table_objects.py`, `odoo/orm/domains.py`.
