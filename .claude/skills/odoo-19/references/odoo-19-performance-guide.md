...
Calling `invalidate_model()`/`invalidate_recordset()` too frequently defeats the purpose of caching.
...
> **Verified against `odoo/orm/models.py`**: `invalidate_cache()` doesn't exist in Odoo 19 (`grep -n "def invalidate_cache"` → 0 matches) — it was replaced by `invalidate_model()`/`invalidate_recordset()`, which is what the example above already uses correctly.
...
> **Verified against `odoo/tests/common.py:604`**: `assertQueryCount(self, default=0, flush=True, **counters)` — the named kwargs are real **user logins** (e.g. `admin`, `demo`, matching the `@users(...)` test decorator), not a special sentinel. `__system__` isn't a recognized login anywhere in the codebase (0 matches) — use an actual login, or the single-value form `assertQueryCount(1211)` if you don't need per-user counts.
...
- Verified against `.repos/odoo` (community, `version_info = (19, 0, 0, FINAL, 0, '')`): `odoo/tools/profiler.py`, `odoo/orm/models.py`, `odoo/tests/common.py`.
