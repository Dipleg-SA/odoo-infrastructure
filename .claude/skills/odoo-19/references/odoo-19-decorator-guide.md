...
> **Gotcha (verified in `decorators.py`)**: `@api.constrains` only triggers if one of its declared fields is present in the `create()`/`write()` call. A field not shown on the view (and therefore never written) will silently never trigger the constraint on creation — override `create()` explicitly if the check must always run.
...
`@api.model_create_multi` makes `create()` accept either a single dict or a list of dicts — callers can keep passing a single dict, but the *implementation* always receives a list, so it can be written to insert everything in one batch instead of looping (verified in `decorators.py`: the wrapper does `if isinstance(vals_list, Mapping): vals_list = [vals_list]` before calling the wrapped method).
...
> **Verified in `decorators.py`**: the method name **must** start with `_` — `assert method.__name__.startswith('_'), "%s: autovacuum methods must be private"`. A public name raises an `AssertionError` at class-definition time.
...
> **`@api.returns` does not exist in Odoo 19 — do not use it.** It is not defined in `odoo/orm/decorators.py` and is not exported from `odoo/api/__init__.py`; using it raises `AttributeError: module 'odoo.api' has no attribute 'returns'`. It was fully removed, not deprecated-but-working like `read_group()`. Verified with `grep -rn "@api.returns" addons/` against both community and enterprise: **0 matches**. The example previously shown here (`message_post` using `@api.returns('mail.message', ...)`) is stale — `message_post` in `addons/mail/models/mail_thread.py` today has **no decorator at all**.
>
> If you need to communicate a method's return type, use a type hint or a docstring instead — there is no runtime decorator for it in Odoo 19.
...
- Verified against `.repos/odoo` (community, `version_info = (19, 0, 0, FINAL, 0, '')`) and `.repos/enterprise`: `odoo/orm/decorators.py`, `addons/mail/models/mail_thread.py`.
