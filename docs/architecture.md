...
**Reverse proxy: Traefik.** Auto-discovery por labels de Docker — cada contenedor se declara una sola vez, sin archivo de config aparte ni reload manual, lo que importa con un stack de diez y pico de servicios. Métricas Prometheus nativas por router y servicio como bonus. Descartado Nginx: exige config por servicio más reload manual, y un exporter aparte para métricas.
