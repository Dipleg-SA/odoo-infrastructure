...
Necesitás subir, bajar, reiniciar o inspeccionar el servicio `odoo` sin tocar el resto del stack. **No es donde operás módulos** — eso vive en [gestionar-modulo](../modulos/gestionar-modulo.md), que para y levanta el servicio como parte de un target `make addons-*` explícito, no de este runbook.
