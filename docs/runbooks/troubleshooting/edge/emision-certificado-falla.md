...
- Si falla por **validación** y no por credencial, subí el `--dns-cloudflare-propagation-seconds` de `stacks/certbot/scripts/cert.sh` (`cmd_issue`): certbot está pidiendo la verificación antes de que el registro TXT haya propagado.
