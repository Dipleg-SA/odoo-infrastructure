...
Tiene que mostrar `dnsmasq` escuchando en `${LOCAL_IP}:53`. Corre en `network_mode: host` (no publica puerto vía Docker), escuchando directo ahí (`listen-address` + `bind-interfaces` de `stacks/dnsmasq/config/dnsmasq.conf`).
