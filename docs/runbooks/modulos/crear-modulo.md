...
- **`--entrypoint odoo`** — sin él, `docker/app/odoo/entrypoint.sh` intercepta cualquier argumento asumiendo que es un flag de instalación (`-i`/`-u`) y le antepone `-c/-d/--db_host`, que rompe el `scaffold` (no toca la base).
- **`-v "$PWD/addons:/scaffold"`** — el servicio monta `./addons` en `/mnt/extra-addons` **read-only** (`docker/app/odoo/compose.yaml`), a propósito: en operación normal Odoo nunca escribe en los addons. Scaffoldear ahí falla con `OSError: [Errno 30] Read-only file system`. Este segundo mount del mismo directorio, en modo escritura, es solo para este comando.
