...
| Protección | `backup` (restic) + pgBackRest dentro de Postgres | `docker/backups/backup/` |
...
Tres niveles, los mismos en las cinco capas: el `compose.yaml` de cada servicio (con su `Dockerfile` y su config al lado, si le hacen falta), un `compose.yaml` agregador por capa que los junta con `include:`, y el `compose.yaml` de cada entorno que decide qué agregador o qué servicios sueltos incluir. `db`, `app` y `observability` son todo-o-nada por entorno, así que los tres entrypoints que las llevan incluyen directo el agregador de la capa. `edge` y `docker/backups/` (protección + restore, misma carpeta porque son complementarios) no: dev solo lleva `nginx` de `edge`, staging lleva `restore-*` pero no `backup` de `backups` — Compose no puede sacar un servicio de un archivo ya incluido, así que el agregador de esas dos capas solo lo usa el entrypoint que las quiere completas (producción); los demás listan el `compose.yaml` de cada servicio suelto.
...
- **`archive_mode` vive en `postgresql.conf`, real por checkout** (gitignoreado, bootstrapeado desde su `.example`, que trae `on`). Un stack sin la capa de backups apunta a la stanza de producción para restaurar: con el archivado prendido le empuja su propio WAL y contamina el repositorio real. `compose.staging.yaml`/`compose.dev.yaml` lo fuerzan a `off` con un `-c` propio —respaldo estructural, no depende de que ese `postgresql.conf` se haya editado— y `db-verify` espera el valor según las capas del stack.
- **`docker/app/odoo/entrypoint.sh` genera config en runtime**. El `addons_path` sale de un glob sobre cuatro categorías en orden de precedencia (`enterprise > custom-addons > oca > third-party`), y `admin_passwd` y `smtp_password` se appendean al conf desde su secret. `smtp_server`/`port`/`user` ya no: son literales de `docker/app/odoo/odoo.conf`, real por checkout.
...
| `KEEP_DAILY`/`WEEKLY`/`MONTHLY` de `scripts/backup.sh` | `repo1-retention-full` de `docker/db/postgres/pgbackrest.conf` | un restore viejo se queda sin adjuntos |
| `RESTIC_MAX_AGE` de `docker/backups/backup/compose.yaml` | `OnCalendar` de `docker/host/systemd/` | el contenedor queda unhealthy para siempre |
...
- **Parametrizá por `.env` solo lo que de verdad varía por deployment y se usa dentro de un `compose.yaml` bajo `docker/`.** Un valor que nunca se tocó en ningún `.env.*.example` real, o que forma pareja con otro archivo (`RESTIC_MAX_AGE` con el `OnCalendar` del timer, la retención de restic con la de pgBackRest), va literal en el compose — la indirección de `.env` no lo hace más seguro, lo hace más fácil de tocar sin la pareja. Cuando el valor sí varía y el archivo de config propio de una herramienta lo puede llevar literal, va ahí, no en `.env`: `docker/edge/nginx/{00-http,server-tls}.conf` y `odoo.locations` son reales, gitignoreados y bootstrapeados con `cp` desde su `.example` — el operador edita el archivo mismo, sin `envsubst` ni variables de por medio. Compose no interpola dentro de archivos bind-mounted, así que cuando la herramienta no ofrece esta vía (pgBackRest no interpola nada y se pisa por variables de entorno opción por opción) el valor todavía tiene que llegar por el mecanismo que *esa herramienta* ofrezca.
- **Los compose viven en `docker/`, el `.env` en la raíz.** Compose lee `.env` del directorio donde se corre el comando, no del que contiene el archivo elegido, así que `COMPOSE_FILE=docker/compose.yaml` alcanza y ningún comando lleva `-f`. Dos consecuencias: sin `COMPOSE_PROJECT_NAME` el fallback del nombre de proyecto es el literal `docker`, igual en toda máquina — dos checkouts sin la variable comparten volúmenes — y, para las rutas relativas de cada compose, `include:` resuelve cada archivo incluido contra **su propia** carpeta, no contra la del que lo incluye. Por eso `secrets:` (declarado solo en los tres entrypoints) sigue en `../secrets/…`, mientras que un servicio como `docker/db/postgres/compose.yaml` monta su config con `./postgresql.conf` — está al lado — y sube tantos `../` como carpetas lo separen de algo compartido (`../../../addons`, `../../../state/…`).
- **La imagen de Odoo lee sus pines de un contexto de build aparte.** `addons/requirements.txt` queda fuera del contexto (`docker/app/odoo`), así que llega por `additional_contexts: {addons: ../../../addons}` y `COPY --from=addons`. El corchete de `requirements.tx[t]` no es un typo: hace el `COPY` opcional —con cero matches, un glob normal aborta el build— para que un checkout sin pines buildee igual. El `RUN` que lo instala va condicionado con `if`, no con `&&`: un `;` ahí dejaría que un `pip install` fallado devuelva 0 y produzca una imagen sin dependencias.
